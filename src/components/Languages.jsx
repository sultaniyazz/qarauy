...
const Languages = () => {
    ...
    return (
        <div onMouseEnter={handleOnMouseEnter} onMouseLeave={handleOnMouseLeave} className={`${isHover ? "shadow-md" : ""} max-md:w-full relative`}>
            <button className={`${isHover ? "rounded-t-[.5vw] bg-white" : "rounded bg-gray-50"} duration-100 max-md:w-full  flex items-center text-[1.2vw] max-md:text-[3vw] max-md:px-[3vw] w-[11.5vw] py-[.2vw]  px-[1.2vw] gap-[.2vw]`}>
                <TbWorld />
            ...
            {isHover && (
                <div className="absolute bg-white rounded-b-[.5vw] w-full  flex flex-col gap-[1vw]">
                    {langNav.map((lang) => (
                        <button key={lang.id} onClick={() => changeLangBtn(lang.title)} className="text-[1.2vw] max-md:text-[3vw] py-[.4vw] px-[1.2vw] active:scale-95 w-full flex items-center gap-[.2vw]">
